...
def travel_to_next_obstacle(current_position, obstacles, room_dimensions):
    ...
    i = 1
    while coords not in obstacles and room_dimensions[0] >= coords[0] > 0 and room_dimensions[1] >= coords[1] > 0:
        footsteps = ()
        if direction == "^":
            coords = [x, y - i]
            footsteps = (x, y - i, "|")
...
def compute_path(data):
    ...
    footsteps = [current_position]

    exited_the_room = False
    while not exited_the_room:
        footsteps += travel_to_next_obstacle(
            current_position, 
            obstacles, 
            room_dimensions
        )
        last_direction = current_position[2]
        last_position_of_path = footsteps[-2]
        current_position = [last_position_of_path[0], last_position_of_path[1], get_next_direction(last_direction)]
    ...
    print_room(room_dimensions, obstacles, footsteps)


def print_room(room_dimensions, obstacles, positions):
    result = 0
    print(positions)
    position_coords = [[pos[0], pos[1]] for pos in positions]
    y = 1
